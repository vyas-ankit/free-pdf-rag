...
def lambda_handler(event, context):
    try:
        # 2. Extract S3 Bucket and Key (File Path) from the trigger event
        ...
        # We only want to process files in the "documents/" folder and ignore temporary files
        ...
        # 3. Pull Pinecone API Key securely from SSM Parameter Store on-the-fly
        ...
        # 4. Download PDF from S3 into Lambda's temporary local folder (/tmp)
        ...
        for doc in documents:
            ...
            doc.metadata["required_role"] = "Public"  # Default fallback role
        ...
        # 7. Clean up local tmp file to free up memory
