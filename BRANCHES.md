...
| `rag_with_mcp_and_agentic_tools` | `06_rag_with_mcp_and_agentic_tools` | Added a single retrieval tool with query expansion and LLM-driven follow-up retrieval, SQLite-backed session persistence, two YAML workflows (book desk, submit vacation), and later removed all LangGraph code in favor of a pure LangChain implementation (`rag_simple.py`). |
| `07_moving_from_langchain_to_langgraph` | `07_moving_from_langchain_to_langgraph` | Current/active branch. Rewrote the RAG + actions control flow as a LangGraph `StateGraph` (`agent_state.py`, `nodes.py`, `graph.py`) backed by a `SqliteSaver` checkpointer, replacing the hand-rolled SQLite `workflow_state` table and in-memory pending-context-switch dict — with no change to `query_rag_simple`'s public behavior. Also added per-user threading to the CLI (returning users resume their history) and a `sessions.db` inspection notebook. |
